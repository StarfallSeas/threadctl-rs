...
threadctl-rs 是一个 **AI 主导的软件工程实验项目**：多模型 AI 协作完成架构、
实现、审查与文档，人类负责方向确认、最终裁决与真实设备验证。

| 角色 | 模型 | 职责 |
|---|---|---|
| 实现 | DeepSeek V4 Flash | 架构设计、代码实现、工程推进 |
| 代码审查 | Claude | 代码级架构审查、缺陷发现 |
| 文档/规范审查 | ChatGPT | 文档审计、工程规范 |
| 人类（维护者） | — | 需求方向、最终决策、真机验证 |

> 责任边界：AI 提供工程能力，人类保留决策权——需求是否采纳、架构是否
> 改动、版本是否发布，均由维护者裁决。详细流程见 `docs/AI-workflow.md`。
...
- 本仓库记录了 AI 协作开发过程；**AI 生成的决策可能包含错误**——所有架构
  决策均需经过源码审查、编译验证与硬件实测，不因"AI 生成"而免除验证。
